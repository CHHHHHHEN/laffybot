...
from laffybot.db.errors import SessionNotFoundError, SessionStateError
...
class SQLiteStore(SessionStore):
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        db_manager.add_schema(_SESSION_SCHEMA_SQL)
    ...
    async def check_connection(self) -> None:
        await self._db_manager.execute("SELECT 1")
    ...
    def _row_to_session(row: aiosqlite.Row) -> SessionInfo:
        return SessionInfo(
            ...
            archived_at=DatabaseManager.parse_dt(row["archived_at"])
            if row["archived_at"]
            else None,
        )
    ...
    async def save_message(
        ...
        db = await self._db_manager.connect()
        timestamp = DatabaseManager.format_dt(DatabaseManager.now())
        metadata_json = (
    ...
    async def get_messages(
        ...
        if before is not None:
            ...
            params.append(DatabaseManager.format_dt(before))
        query = (
    ...
    async def replace_compressed_region(
        ...
        try:
            ...
            latest_ts = (
                ...
                else DatabaseManager.format_dt(DatabaseManager.now())
            )
            ...
            await db.execute(
                """UPDATE sessions
                   ...
                   WHERE session_id = ?""",
                (delta, DatabaseManager.format_dt(DatabaseManager.now()), session_id),
            )
            await db.commit()
            logger.debug(
