"""OpenAI-compatible provider for LLM APIs."""
...
_DEFAULT_OPENROUTER_HEADERS = {
    "HTTP-Referer": "https://github.com/laffybot",
    "X-OpenRouter-Title": "laffybot",
...
def _openai_compat_timeout_s() -> float:
    return _float_env(
        "LAFFYBOT_OPENAI_COMPAT_TIMEOUT_S", _OPENAI_COMPAT_REQUEST_TIMEOUT_S
    )


def _stream_idle_timeout_s() -> int:
    return int(
        os.environ.get("LAFFYBOT_STREAM_IDLE_TIMEOUT_S", str(_STREAM_IDLE_TIMEOUT_S))
    )
...
class OpenAIProvider(BaseProvider):
    def __init__(self, config: ProviderConfig):
        ...
        self._extra_headers = config.extra_headers or {}
        self._extra_body = config.extra_body or {}

        default_headers: dict[str, str] = {}
        if _uses_openrouter_attribution(self._api_base):
        ...
        if _is_local_endpoint(self._api_base):
    ...
    def _sanitize_empty_content(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for msg in messages:
            ...
            if content is None and clean.get("role") != "assistant":
                clean["content"] = ""
            result.append(clean)
        return result
        ...
        return LLMResponse(
            ...
            tool_calls=[
                ToolCallRequest(
                    ...
                    arguments=(
                        parsed
                        if isinstance(parsed := json_repair.loads(b["arguments"]), dict)
                        else {}
                    ),
                    extra_content=b.get("extra_content"),
                    provider_specific_fields=b.get("prov") or b.get("fn_prov"),
                )
    ...
    def _extract_error_metadata(cls, e: Exception) -> dict[str, Any]:
        ...
        if "timeout" in error_name:
            ...
            error_kind = ERROR_CONNECTION
        elif status_code is not None:
            if status_code == 429:
                error_kind = ERROR_RATE_LIMIT
