...
- **AgentRunner 单例**：所有会话共享同一个 AgentRunner 实例，Provider 连接复用
- **SessionStore 依赖注入**：SessionStore 作为依赖注入，支持不同存储后端
- **ContextBuilder 职责分离**：将消息历史管理拆分为存储层和构建层，两组件通过清晰接口协作（详见 `context-builder-design.md`）
...
### 4. 上下文构建
历史消息查询（支持容量控制）、系统提示组装、完整上下文构建

### 5. 资源管理
AgentRunner 实例管理、CancellationToken 生命周期
...
- `model: str` - LLM 模型标识符
- `status: str` - 会话状态（idle/busy/error/inactive）
...
**返回：** 保存的消息字典（包含 role、content、timestamp、metadata）
...
**参数：** `store` - 会话持久化存储；`context_builder` - 上下文构建器（参见 context-builder-design.md）；`tool_registry` - 工具注册表；`provider_factory` - LLM 提供者工厂函数
...
**异常：** `SessionNotFoundError` - 会话不存在；`SessionBusyError` - 会话忙碌；`SessionInactiveError` - 会话已失效
...
适配不同 LLM 的上下文窗口限制，避免 token 超限，减少无关历史对推理质量的干扰。
...
**参数：** `status` - 可选的状态过滤；`limit` - 最大返回会话数；`offset` - 分页偏移
...
全局唯一 AgentRunner 实例，所有会话共享 Provider 连接池。AgentRunner 内部状态无共享，支持并发调用。

**优势**：资源效率高（避免重复创建连接）、连接复用（HTTP 连接池共享）、简化管理（无需实例缓存和清理逻辑）。

### 消息格式转换

#### 转换层次
```
API 层消息格式 → SessionManager 转换 → 存储格式（SessionStore） → ContextBuilder 构建 → AgentRunner 消息格式（AgentRunSpec）
```

#### 转换职责
- **API → 存储**：添加时间戳、元数据，持久化用户消息
- **存储 → AgentRunner**：ContextBuilder 加载历史消息，构造完整的对话上下文
...
2. 构建上下文 → 调用 context_builder.build_messages()，内部调用 SessionStore.get_messages() 获取历史，返回完整消息列表（系统提示 + 历史 + 当前消息），详见 context-builder-design.md
3. 执行 LLM → 生成 request_id、创建 CancellationToken、更新会话状态为 busy、保存用户消息、调用 AgentRunner.run_stream()、转发 SSE 事件
...
**数据结构**：`cancelled: bool`（取消标志）、`callbacks: list[Callable]`（取消回调列表）

**核心操作**：`cancel(reason)`（触发取消，执行所有回调）、`is_cancelled()`（检查是否已取消）、`register_callback(callback)`（注册清理回调）

**设计要点**：轻量级实现，无异步开销；支持回调注册，用于资源清理；线程安全（单实例部署，无跨线程访问）。

#### 协调流程
```
用户调用 cancel_request() → SessionManager 获取会话锁 → 验证状态为 busy → 获取 CancellationToken → 调用 token.cancel(reason) → 返回 request_id
AgentRunner 执行中 → 在检查点调用 token.is_cancelled() → 检测到取消 → 抛出 CancelledError → SessionManager 捕获并生成 cancelled 事件
```

#### 检查点机制

**AgentRunner 检查点位置**：LLM API 调用前、工具调用前、流式响应间隙（每接收一个 chunk）、迭代循环开始。

**检查策略**：每个检查点调用 `token.is_cancelled()`，检测到取消立即抛出 `CancelledError`，避免在不可中断操作中检查（如数据库事务）。

#### 信号传播路径
```
SessionManager 持有 active_tokens: dict[session_id, CancellationToken] → send_message() 时创建 token 并存储 → cancel_request() 时获取并触发 token.cancel()
  ↓
AgentRunner 接收 token 作为参数 → 在检查点调用 token.is_cancelled() → 检测到取消 → 抛出 CancelledError
  ↓
SessionManager 捕获 CancelledError → 生成 cancelled 事件 → 更新会话状态 → idle → 清理 token
```
...
- **AgentRunner 实例**：全局单例，所有会话共享，生命周期与应用一致
