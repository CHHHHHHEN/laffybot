...
- `model: str` - LLM 模型标识符
- `status: SessionStatus` - 会话状态（idle/busy/error/inactive），类型为 `Literal["idle", "busy", "error", "inactive"]`
...
**参数：** `status` - 可选的状态过滤，默认过滤掉 `inactive` 状态的会话
...
3. 执行 LLM → 生成 request_id、创建 CancellationToken、更新会话状态为 busy、保存用户消息、创建 AgentRunner 实例、调用 AgentRunner.run_stream()、转发 SSE 事件
