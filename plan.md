...
- 所有测试文件在模块级 fixture 中静默 `loguru`：
  ...
  ```
- 仅当测试明确验证日志行为时使用 `pytest.LogCaptureFixture`（caplog）
...
| 依赖 | 必要性 | 用途 | 安装方式 |
|------|--------|------|---------|
| `respx` | **推荐** | Mock httpx 请求：模拟 OpenAI API 的各种错误响应（timeout、429、5xx）、模拟 MCP SSE/HTTP 传输层的网络交互。避免手工 mock `AsyncOpenAI` / `httpx.AsyncClient`，同时保留 httpx 的错误模型 | `uv add --dev respx` |
...
所有共享 fixture 在此定义，作为团队契约。标注 `# TODO: implement` 的 fixture 需在编写第一个使用它的测试文件之前完成实现。
...
def mock_provider() -> _MockProvider:
    ...
    - chat_completion(**kwargs) → SuccessLLMResponse | ErrorLLMResponse
      - 可通过 .set_chat_completion_response(response) 预设返回值
      - 可通过 .set_chat_completion_error(error_kind, message) 预设错误
      - 未预设时返回 SuccessLLMResponse(content="hello")
    - chat_completion_stream(messages, model, on_chunk, ...) → SuccessLLMResponse | ErrorLLMResponse
      - 可通过 .set_stream_chunks(chunks: list[StreamChunk]) 预设流式 chunk 序列
      - 每个 chunk 会依次调用 on_chunk(chunk)
      - 流结束后返回预设的 final response
      - 未预设时返回 SuccessLLMResponse(content="")
    - 每次调用记录 messages 和 model 到 .call_history: list[dict]
    """
```
...
def mock_transport() -> _MockTransport:
    ...
    - receive() → str
      - 从预定义队列返回消息; 队列空时阻塞直到有消息或超时
      - 可通过 .queue_response(data: str) 预置响应
      - 可通过 .queue_disconnect() 使下次 receive() 抛出 TransportError
    - close() → None
...
```
Phase 0-1: mock_tool + tool_registry      (无外部依赖，最先实现)
Phase 0-2: context_config + file_states   (零外部依赖，Pydantic / 纯内存)
Phase 0-3: mock_provider                  (runner, openai, compressor 等依赖)
Phase 0-4: mock_transport                 (全部 MCP 测试依赖)
Phase 0-5: mcp_server_config              (manager.py 测试依赖)
```

`mock_provider` 和 `mock_transport` 的复杂性最高。Phase 0-3 和 0-4 可各安排 1 人。

**Phase 0 验收标准**：`conftest.py` 中所有 fixture 实现完成，签名和行为符合 §6.1 契约。
**Phase 0 门禁**：编写任何测试文件之前，Phase 0 必须先完成并通过 code review。
...
### Phase 0：基础设施（独占，不可并行）

编写全部共享 fixture 到 `tests/conftest.py`。顺序见 §6.2。

**门禁**：Phase 0 完成前，不得开始任何测试文件的编写。
...
|------|---------|------|
| `test_events.py` | ~40 | 无 |
| `test_config.py` (ContextConfig) | ~10 | 无 |
| `tools/test_file_state.py` | ~15 | 无 |
| `tools/test_base.py` | ~12 | 无 |

这 4 个文件无相互依赖，4 人可各认领一个并行编写。
**完成标准**：`pytest` 通过，`ruff check` / `mypy` 通过。

### Phase 2：核心执行（依赖 Phase 1）

| 文件 | 估算用例 | 前置 |
|------|---------|------|
| `test_heartbeat.py` | ~15 | events |
| `test_runner.py` | ~30 | events + cancellation + registry + providers/base |

**完成标准**：Phase 1 全部测试 + 本阶段测试通过。

### Phase 3：文件系统 + Shell（依赖 Phase 1）

| 文件 | 估算用例 | 前置 |
|------|---------|------|
| `tools/test_filesystem.py` | ~50 | file_state + base |
| `test_shell.py` | ~20 | base |

这两个文件无相互依赖，可并行。
**完成标准**：Phase 1 全部测试 + 本阶段测试通过。

### Phase 4：OpenAI Provider（依赖 Phase 1）

| 文件 | 估算用例 | 前置 |
|------|---------|------|
| `providers/test_openai.py` | ~40 | base + types + config |

**完成标准**：Phase 0–1 全部测试 + 本阶段测试通过。

### Phase 5：MCP 全栈（依赖 Phase 1 + mock_transport）

| 文件 | 估算用例 | 前置 |
|------|---------|------|
| `tools/mcp/test_client.py` | ~20 | base + mock_transport |
| `tools/mcp/test_transports.py` | ~20 | 无（独立 I/O 测试） |
| `tools/mcp/test_wrappers.py` | ~15 | base |
| `tools/mcp/test_manager.py` | ~15 | client + transport + wrappers |

`test_wrappers` 与其他 3 个无依赖，可并行。`test_transports` 也可独立编写。
**完成标准**：Phase 0–1 全部测试 + 本阶段测试通过。

### Phase 6：Context + Skills（依赖 Phase 1）

| 文件 | 估算用例 | 前置 |
|------|---------|------|
| `context/test_templates.py` | ~10 | config |
| `context/test_builder.py` | ~10 | config + types + tokens + ToolRegistry |
| `context/test_compressor.py` | ~20 | config + types + tokens + ToolRegistry + providers/base |
| `skills/test_loader.py` | ~10 | models（零依赖） |
| `skills/test_registry.py` | ~5 | loader + SkillRegistryStore |
| `tools/test_skill_view.py` | ~5 | loader + registry |

`test_templates` / `test_loader` 无相互依赖，可并行。
**完成标准**：Phase 0–1 全部测试 + 本阶段测试通过。
...
| 文件 | 估算用例 | 前置 |
|------|---------|------|
| `test_title_generator.py` | ~10 | providers/base |
| `providers/test_config.py` | ~3 | 无 |
| `providers/test_errors.py` | ~5 | 无 |
| `providers/test_base.py` | ~3 | 无 |
| `providers/test_factory.py` | ~3 | 无 |
| `skills/test_errors.py` | ~2 | 无 |
| `tools/test_errors.py` | ~3 | 无 |
| `context/test_types.py` | ~2 | 无 |
| `tools/mcp/test_mcp_init.py` | ~2 | 无 |
| `__init__.py` 导出测试 | ~5 | 全部模块 |

**完成标准**：全部测试通过，覆盖率 ≥ 75%。

**总估算：~380 个新增用例**（现有 34 个 → 目标 ~414 个）。

### 验收标准（每阶段）

| 阶段 | 验收标准 | 预计工时（1人） |
|------|---------|---------------|
| **Phase 0** | `conftest.py` 所有 fixture 实现完成，`ruff check` / `mypy` 通过。编写任何测试文件前必须完成 | ~2h |
| **Phase 1** | 4 个测试文件全部通过。所有 fixture 的每个契约路径至少被一个测试覆盖 | ~3h |
| **Phase 2** | heartbeat + runner 全部测试通过。runner 测试覆盖 4 种事件序列 + 3 种错误路径 + 取消路径 | ~4h |
| **Phase 3** | filesystem + shell 全部测试通过。filesystem 测试覆盖 read/write/edit/list 四工具的所有子用例 | ~6h |
| **Phase 4** | OpenAI provider 全部测试通过。非流式 + 流式 + 消息清洗三组均覆盖 | ~4h |
| **Phase 5** | 4 个 MCP 测试文件全部通过。JSON-RPC 序列与 §3.3 规范一致 | ~5h |
| **Phase 6** | context + skills 全部测试通过。覆盖率（指令）≥ 60% | ~3h |
| **Phase 7** | 全部测试通过。覆盖率（指令）≥ 75%。`__init__.py` 导出符号与模块内容一致 | ~2h |
